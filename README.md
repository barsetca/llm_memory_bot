...
Консольное приложение для общения с OpenAI с сохранением контекста в SQLite.
...
cp .env.example .env
# Заполните .env: OPENAI_API_KEY; при необходимости OPENAI_MODEL (по умолчанию gpt-4o-mini); для бота — TELEGRAM_BOT_TOKEN
```
...
- **Начать диалог** — следующий текст пользователя отправляется модели, тезисы сохраняются в контекст этого пользователя, ответ выводится в чат.
